...
glyph () {
	local uri="$1" 
	if curl -s "$GLYPH_RESOLVER/health" > /dev/null 2>&1
	then
		curl -s "$GLYPH_RESOLVER/resolve?uri=$uri" | jq .
	else
		curl -s "$GLYPH_RESOLVER_FALLBACK/resolve?uri=$uri" | jq .
	fi
}
glyph-register () {
	local id="$1" 
	local type="${2:-agent}" 
	local location="${3:-fractal://local}" 
	curl -X POST "$GLYPH_RESOLVER/register" -H "Content-Type: application/json" -d "{
            \"glyph_id\": \"$id\",
            \"entity_type\": \"$type\",
            \"location_uri\": \"$location\",
            \"location_type\": \"fractal\"
        }"
}
is-at-least () {
	emulate -L zsh
...
alias -- glyph-resolve=glyph
alias -- glyph-self='glyph "glyph://$GLYPH_IDENTITY"'
alias -- glyph-skull='glyph "glyph://skull-core.⟁"'
alias -- run-help=man
...
export PATH=/Users/chaoshex/.deno/bin\:/Users/chaoshex/Library/pnpm\:/Library/Frameworks/Python.framework/Versions/3.13/bin\:/opt/homebrew/bin\:/opt/homebrew/sbin\:/usr/local/bin\:/System/Cryptexes/App/usr/bin\:/usr/bin\:/bin\:/usr/sbin\:/sbin\:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/local/bin\:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/bin\:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/appleinternal/bin\:/Users/chaoshex/.local/bin\:/Users/chaoshex/Library/Python/3.13/bin\:/Users/chaoshex/Library/Python/3.13/bin
